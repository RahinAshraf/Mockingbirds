...
class SummaryJourneyScreenState extends State<SummaryJourneyScreen> {
  ...
  _setData() async {
    var user = await _databaseManager.getByKey(
        'users', _databaseManager.getCurrentUser()!.uid);
    var group = await _databaseManager.getByEquality('group', 'code', user.data()!['code']);
    var tempOrganiser;
    group.docs.forEach((element) {
      tempOrganiser = element.data()['organiser'];
    });
    setState(() {
      isInGroup = user.data()!.keys.contains('group');
      if(isInGroup){
        organiser = tempOrganiser;
      }else{
  ...
  _leaveGroup() async {
    try {
      ...
      for (var element in temp.docs) {
        ...
        print(list);
        print(" tototo" + list.isEmpty.toString());
        if (list.isEmpty) {
