...
void main() {
  ...
  test('deleting group works', () async {
    DocumentSnapshot<
        Map<String, dynamic>> userResponse = MockDocumentSnapshot();
    QuerySnapshot<Map<String, dynamic>> groupResponse = MockQuerySnapshot();
    var userID = "test";
    var groupCode = "groupCode";
    QueryDocumentSnapshot<
        Map<String, dynamic>> groupdoc = MockQueryDocumentSnapshot();
    List<QueryDocumentSnapshot<Map<String, dynamic>>> list = [];
    DocumentReference<
        Map<String, dynamic>> groupDocRef = MockDocumentReference();
    list.add(groupdoc);
    List<String> memberList = [userID];
    Map<String, dynamic> userMap = {'group': groupCode};
    Map<String, dynamic> groupMap = {
      'createdAt': Timestamp.fromDate(Timestamp.now().toDate().subtract( Duration(days:3))),
      'memberList': memberList
    };
    var userKeys = ['group'];
    when(_databaseManager.getCurrentUser()).thenReturn(user);
    when(user.uid).thenReturn(userID);
    when(_databaseManager.getByKey(
        'users', userID)).thenAnswer((_) async => userResponse);
    when(userResponse.data()).thenReturn(userMap);
    when(_databaseManager.getByEquality(
        'group', 'code', groupCode)).thenAnswer((_) async => groupResponse);
    ...
    verify(_databaseManager.setByKey(
        'users', userID, any,any)).called(1);
  }
